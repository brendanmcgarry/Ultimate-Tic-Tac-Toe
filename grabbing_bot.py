...
class bot:
    ...
    def __init__(self):
        self.team_name = "GrabbingBot"
        self.tick = None
    
    def move(self, board, forced_moves, start_move=None):
        """Logic for your bot"""
        if not self.tick:
            self._set_tick(board)
        
        # if self.get_move_num(board) == 1:
        #     return 'NW', 'NW'
    ...
    def get_move_num(self, board):
        return len([a for b in board for a in b if a != EMPTY]) + 1
