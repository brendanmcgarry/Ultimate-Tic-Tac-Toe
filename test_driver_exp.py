...
if __name__ == "__main__":
    ...
    start_moves = bots[0].get_all_moves(Game().board, list(Game.map_tile))
    start_move_strens = {start_move: [] for start_move in start_moves}
    ...
    for game_num in range(rounds):
        ...
        for _ in range(num_sub_rounds):
            ...
            while True:
                ...
                for _ in range(10):
                    if move_num == 0:
                        start_move = start_moves[game_num]
                    else:
                        start_move = None
                    player_move = bots[game.player_turn].move(
                        copy.deepcopy(game.board), copy.deepcopy(forced_move), start_move=start_move
                    )
            ...
            if draw:
                start_move_strens[start_moves[game_num]].append(0.5)
            elif game.player_turn == 0:
                start_move_strens[start_moves[game_num]].append(1)
            elif game.player_turn == 1:
                start_move_strens[start_moves[game_num]].append(0)
            # c = Counter(winners)
            # print(c)
            # for b in range(2):
    ...
    print({move: sum(start_move_strens[move]) / num_sub_rounds for move in start_move_strens})
    c = Counter(winners)
    print(c)
    for b in range(2):
        print('{}: {}%'.format(bots[b].team_name,
